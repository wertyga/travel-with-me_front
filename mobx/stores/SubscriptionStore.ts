...
import {
	...
	createSubscriptionPayment, donationPaymentApi,
	fetchMySubscription,
	fetchSubscriptionList,
	renewMySubscription as renewMySubscriptionApi,
} from '@/api';
import { withLoading } from '../store.utils';
import { CreateSubscriptionResponse, Price, RootStoreType, SubscriptionPreview, UserSubscription } from '@/types';
...
export class SubscriptionStore {
  ...
  constructor(public rootStore: RootStoreType) {
	...
	async createDonationPaymentSheet(price: Price, payeeId: string) {
		const res = await donationPaymentApi(price, payeeId)
	}
}
